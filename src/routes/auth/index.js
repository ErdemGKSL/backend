...
const { getUserBlockReason } = require('../../other/blocked-users.js');
...
router.get('/exchange', async (req, res) => {
  ...
  try {
    ...
    let blockReason = getUserBlockReason(user.id);

    if (blockReason) return res.send({ ok: false, error: `Blocked: ${blockReason}` });
    ...
    res.send({ ok: false, error: 'Invalid acordToken provided.' });
  }
});
